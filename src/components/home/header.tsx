        ...
        {socials.map((s,i)=>{
            return (
                <React.Fragment key={i}>
                    <Link title={s.name} href={s.url} target='_blank' className='group  relative text-xs text-bg dark:bg-zinc-300 p-1 rounded-sm bg-neutral-800  flex items-center gap-1.5'>
                        {
                            typeof s.icon === 'string'
                            ?
                            <Image alt={s.name} width={14} height={14} className='dark:mix-blend-difference invert' src={s.icon}/>
                            :
                            <s.icon size={14} className='text-zinc-100 dark:text-neutral-900' />
                        }
                    </Link >
                </React.Fragment>
